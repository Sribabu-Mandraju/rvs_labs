...
function Deposit() {
  ...
  const isValidToken =
    selectedToken !== "" && publicMetaData?.allowedTokens?.some((token) => token.address === selectedToken)

  const depositAmount = isValidAmount ? BigInt(Math.floor(Number(amount) * 10 ** 6)) : BigInt(0)
  ...
  const handleDeposit = async () => {
    ...
    if (!hasEnoughBalance) {
      toast.error(`Insufficient token balance (need ${Number(depositAmount) / 10 ** 6} tokens).`, {
        id: "balance-error",
      })
      return
    }
    ...
    if (tokenBalanceError) {
      toast.error(`Error checking token balance: ${tokenBalanceError.message.slice(0, 100)}...`, {
        id: "balance-error",
      })
      return
    }
  ...
  const formatAmount = (value) => (value > 0 ? (Number(value) / 10 ** 6).toFixed(2) : "0.00")

  // Get token name for display
  const getTokenName = (tokenAddress) => {
    const token = publicMetaData?.allowedTokens?.find((t) => t.address === tokenAddress)
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black relative overflow-hidden">
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl lg:text-5xl font-bold text-white mb-4">Stake Your Tokens</h1>
          <p className="text-xl text-gray-400 max-w-2xl mx-auto">
            Earn rewards by staking your tokens with our secure TimeLock NFT system
          </p>
        </div>
            ...
            <div className="bg-gray-800/50 backdrop-blur-xl border border-gray-700/50 rounded-2xl p-6 shadow-2xl">
              ...
              {isConnected ? (
                <div className="space-y-3">
                  ...
                  <div className="bg-gray-700/50 rounded-lg p-3">
                    ...
                    <p className="text-sm text-white">
                      {balanceData ? (Number(balanceData.value) / 10 ** 18).toFixed(4) : "0.0000"} ETH
                    </p>
                  </div>
                  {!isCorrectNetwork && (
                ...
                <div className="text-center py-4">
                  ...
                  <p className="text-sm text-gray-400">Please connect your wallet</p>
                </div>
              )}
            </div>
            ...
            {publicMetaData && (
              <div className="bg-gray-800/50 backdrop-blur-xl border border-gray-700/50 rounded-2xl p-6 shadow-2xl">
                <div className="flex items-center space-x-3 mb-4">
                  ...
                  <h3 className="text-lg font-semibold text-white">ROI Rates</h3>
                </div>
                <div className="space-y-3">
                  {[
                    { period: "1 Month", roi: publicMetaData.roi1m, icon: "🚀" },
                    { period: "2 Months", roi: publicMetaData.roi2m, icon: "💎" },
                    { period: "3 Months", roi: publicMetaData.roi3m, icon: "👑" },
                  ].map((item, index) => (
            ...
            <div className="bg-gray-800/50 backdrop-blur-xl border border-gray-700/50 rounded-2xl p-8 shadow-2xl">
              <div className="flex items-center space-x-3 mb-8">
                ...
                <div>
                  ...
                  <p className="text-gray-400">Lock your tokens to earn rewards</p>
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6 mb-8">
                ...
                <div className="space-y-2">
                  ...
                  {selectedToken && (
                    <p className="text-xs text-gray-400">
                      Balance: {formatAmount(tokenBalance)} {getTokenName(selectedToken)}
                    </p>
                  )}
                </div>
                ...
                {/* Lock Period */}
                <div className="space-y-2">
                  <label className="block text-sm font-medium text-gray-300">Lock Period</label>
                  <div className="relative">
                    <select
                      value={periodMonths}
                      ...
                      className="w-full p-4 rounded-xl bg-gray-700/50 text-white border border-gray-600/50 focus:border-yellow-400 focus:ring-2 focus:ring-yellow-400/20 transition-all duration-300 appearance-none cursor-pointer"
                    >
                      <option value="1">1 Month ({(Number(publicMetaData?.roi1m || 0) / 100).toFixed(2)}% ROI)</option>
                      <option value="2">2 Months ({(Number(publicMetaData?.roi2m || 0) / 100).toFixed(2)}% ROI)</option>
                      <option value="3">3 Months ({(Number(publicMetaData?.roi3m || 0) / 100).toFixed(2)}% ROI)</option>
                    </select>
              ...
              <div className="space-y-2 mb-8">
                <label className="block text-sm font-medium text-gray-300">Deposit Amount</label>
                <div className="relative">
                  <input
                    type="number"
                    value={amount}
              ...
              {!isConnected && (
                <div className="mt-4 p-3 bg-red-500/10 border border-red-500/20 rounded-lg">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-red-400">Please connect your wallet to continue</span>
                  </div>
                </div>
              )}
              ...
              {isConnected && !isCorrectNetwork && (
                <div className="mt-4 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-yellow-400">Please switch to Base Sepolia network</span>
                  </div>
                </div>
              )}
            </div>
        ...
        {publicMetaData && (
          <div className="mt-12">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-400">Current staking statistics and information</p>
            </div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-gray-800/50 backdrop-blur-xl border border-gray-700/50 rounded-2xl p-6 shadow-2xl">
                <div className="flex items-center space-x-3 mb-4">
                  ...
                  <h3 className="text-lg font-semibold text-white">Supported Tokens</h3>
                </div>
                <div className="space-y-2">
              ...
              <div className="bg-gray-800/50 backdrop-blur-xl border border-gray-700/50 rounded-2xl p-6 shadow-2xl">
                ...
                <div className="space-y-2">
                  {publicMetaData.depositedBalances.map((bal, index) => (
                    <div key={bal.token} className="flex items-center justify-between p-2 bg-gray-700/30 rounded-lg">
                      <span className="text-sm text-gray-300">{bal.name}</span>
                      <span className="text-sm font-semibold text-white">{formatAmount(bal.balance)}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-gray-800/50 backdrop-blur-xl border border-gray-700/50 rounded-2xl p-6 shadow-2xl md:col-span-2 lg:col-span-1">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-semibold text-white">1-3 Months</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-400">Security</span>
                    <span className="text-sm font-semibold text-yellow-400">NFT Locked</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        {!publicMetaData && (
          <div className="flex items-center justify-center py-12">
            <div className="flex items-center space-x-3">
              ...
              <span className="text-lg text-gray-400">Loading platform data...</span>
            </div>
          </div>
        )}
      </main>
      ...
      <footer className="relative z-10 border-t border-gray-700/50 bg-black/20 backdrop-blur-xl mt-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="text-center">
            <p className="text-sm text-gray-500">© 2025 River Labs IT Solutions. All rights reserved.</p>
            <p className="text-xs text-gray-600 mt-1">Secure staking powered by blockchain technology</p>
          </div>
        </div>
      </footer>
    </div>
